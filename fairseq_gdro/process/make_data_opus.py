...
def is_misaligned(l1, l2):
    if len(l1.split()) == 0 or len(l2.split()) == 0:
        return True
    if (
        len(l1.split()) * 1.0 / len(l2.split()) >= 3
        or len(l2.split()) * 1.0 / len(l1.split()) >= 3
    ):
        return True
    return False
...
def main():
    ...
    for d in domains:
        p1, p2 = os.path.join(root_dir, d, "train.en-de.en"), os.path.join(
            root_dir, d, "train.en-de.de"
        )
        train_data[d] = read_data(p1, p2)

        p1, p2 = os.path.join(root_dir, d, "valid.en-de.en"), os.path.join(
            root_dir, d, "valid.en-de.de"
        )
    ...
    with open(os.path.join(opt_dir, train_fsrc), "w", encoding="utf-8") as fout1, open(
        os.path.join(opt_dir, train_ftgt), "w", encoding="utf-8"
    ) as fout2, open(
        os.path.join(opt_dir, "train.labels"), "w", encoding="utf-8"
    ) as fout3:
        for ii, d in enumerate(domains):
            ...
            sep_valid_src = open(
                os.path.join(sep_opt_dir, valid_fsrc), "w", encoding="utf-8"
            )
            sep_valid_tgt = open(
                os.path.join(sep_opt_dir, valid_ftgt), "w", encoding="utf-8"
            )
            ...
            shutil.copyfile(
                os.path.join(root_dir, d, test_fsrc),
                os.path.join(sep_opt_dir, test_fsrc),
            )
            shutil.copyfile(
                os.path.join(root_dir, d, test_ftgt),
                os.path.join(sep_opt_dir, test_ftgt),
            )
    ...
    with open(os.path.join(opt_dir, valid_fsrc), "w", encoding="utf-8") as fout1, open(
        os.path.join(opt_dir, valid_ftgt), "w", encoding="utf-8"
    ) as fout2:
    ...
    for d in domains:
        shutil.copyfile(
            os.path.join(root_dir, d, test_fsrc),
            os.path.join(opt_dir, "{}.{}".format(d, test_fsrc)),
        )
        shutil.copyfile(
            os.path.join(root_dir, d, test_ftgt),
            os.path.join(opt_dir, "{}.{}".format(d, test_ftgt)),
        )
