...
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def compute_group_avg(self, losses, group_idx):
        ...
        group_map = (
            group_idx
            == torch.arange(self.n_train_groups).unsqueeze(1).long().to(self.device)
        ).float()  # G X B
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if len(logging_outputs) > 0 and "nll_0" in logging_outputs[0]:
            gpu_counts = utils.item(
                sum(log.get("gpu_count", 0) for log in logging_outputs)
            )
            for ii in range(ngroups):
                group_loss_token = (
                    sum(log.get("nll_{}".format(ii), 0) for log in logging_outputs)
                    / gpu_counts
                )
                metrics.log_scalar(
                    "gnll_{}".format(ii), group_loss_token / math.log(2), 1, round=3
                )
                metrics.log_scalar(
                    "gppl_{}".format(ii),
                    2 ** (group_loss_token / math.log(2)),
                    0,
                    round=3,
                )
