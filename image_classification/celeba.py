...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="learning rate for exponential decay DRO",
    )
    parser.add_argument(
        "--alpha", type=float, default=0.2, help="alpha for greedy group DRO"
    )
    parser.add_argument(
        "--beta", type=float, default=0.1, help="beta for greedy group DRO"
    )
    parser.add_argument("--eg_normalize", action="store_true", default=False)
    parser.add_argument(
        "--lr_decay",
        choices=["milestone", None],
        default=None,
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=1e-4, help="weight for l2 norm decay"
    )
    ...
    parser.add_argument(
        "--data_path", help="path for data file.", default="data/celeba"
    )
    parser.add_argument(
        "--model_path", help="path for saving model file.", default="models/1_erm"
    )

    parser.add_argument(
        "--loss", choices=["erm", "dro_eg", "dro_greedy"], default="erm"
    )
    parser.add_argument("--reweight", action="store_true", default=False)
    parser.add_argument("--recompute", type=int, default=0)
    parser.add_argument(
        "--group_split",
        choices=["confounder", "domain"],
        default="confounder",
        help="the type of splitting groups",
    )
...
def get_optimizer(
    lr, parameters, momentum, lr_decay, decay_rate, milestone, weight_decay
):
    optimizer = SGD(
        parameters, lr=lr, momentum=momentum, weight_decay=weight_decay, nesterov=True
    )
    opt = "momentum={:.1f}, wd={:.1e}".format(momentum, weight_decay)
    if lr_decay == "milestone":
        opt = opt + "lr decay={} {}, decay rate={:.3f}, ".format(
...
def setup(args):
        ...
        transform=transforms.Compose(
            [
                transforms.RandomResizedCrop(
                    (224, 224), scale=(0.7, 1.0), ratio=(1.0, 4 / 3), interpolation=2
                ),
    ...
    logging(
        "Data size: training: {}, val: {}, test: {}".format(
...
def train(args, train_loader, num_train, model, optimizer):
    ...
    for step, (x, y, g, d, w) in enumerate(train_loader):
        ...
        if step % args.log_interval == 0:
            ...
            log_info = "[{}/{} ({:.0f}%)] loss: {:.4f}, acc: {:.2f}%, ".format(
                accum_loss.count,
                num_train,
                100.0 * accum_loss.count / num_train,
                accum_loss.avg,
                accum_acc.avg,
            )
            log_info += ", ".join(
                [
                    "g{} ({:.3f}, {:.2f}%)".format(
                        i, accum_group_losses[i].avg, accum_group_accs[i].avg
                    )
                    for i in range(n_groups)
                ]
            )
    ...
    log_info = "Average loss: {:.4f}, acc: {:.2f}%, ".format(
        accum_loss.avg, accum_acc.avg
    )
    log_info += ", ".join(
        [
            "g{} ({:.3f}, {:.2f}%)".format(
...
def main(args):
    ...
    logging(
        "# of Parameters: %d" % sum([param.numel() for param in model.parameters()]),
        args.log,
    )

    train_loader, train_eval_loader, val_loader, test_loader = init_dataloader(
    ...
    optimizer, scheduler, opt_param = get_optimizer(
        args.lr,
        model.parameters(),
        momentum,
        lr_decay=lr_decay,
    ...
    for epoch in range(1, epochs + 1):
        ...
        with torch.no_grad():
            ...
            log_info += ", ".join(
                [
                    "g{} ({:.3f}, {:.2f}%)".format(i, group_losses[i], group_accs[i])
                    for i in range(n_groups)
                ]
            )
            log_info += (
                ", robust: {:.2f}%, | best epoch: {} ({:.2f}%), patient: {}".format(
                    robust_acc, best_epoch, best_robust_acc, patient
                )
            )
            ...
            if args.loss == "dro_eg":
                ws_info = ", ".join(
                    [
                        "w{}={:.3f}".format(i, w.item())
                        for i, w in enumerate(model.adv_probs)
                    ]
                )
                ...
                ws_info = ", ".join(
                    [
                        "w{}={:.3f}".format(i, w.item())
                        for i, w in enumerate(model.h_fun)
                    ]
                )
                logging("Group weights: {}".format(ws_info), logfile=log)
                gloss_info = ", ".join(
                    [
                        "g{}={:.3f}".format(i, gs.item())
                        for i, gs in enumerate(model.sum_losses)
                    ]
                )
            ...
            if args.recompute > 0:
                logging(
                    "***************Re-Compute at the end of epoch {}*****************".format(
                        epoch
                    ),
                    log,
                )
